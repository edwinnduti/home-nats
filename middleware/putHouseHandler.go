...
// UPDATE HANLDER
func (srv Server) PutHouseHandler(w http.ResponseWriter, r *http.Request) {
	mapForId := mux.Vars(r)
	...
	msg, err := srv.Nc.Request("putHouse", idMsgInBytes, time.Second)
	if err != nil {
		lib.CheckErr(w, "NATS putHouse request Error", http.StatusInternalServerError, "NATS request Error", err)
	}

	// create log for request performed
	consts.InfoLogger.Printf("%v : %v OK / HOUSE ID %v UPDATED", http.MethodGet, http.StatusOK, id)
